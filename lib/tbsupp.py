...
def describe_rsync_exit_code(code):
    ...
    return({
        ...
        }.get(int(code), 'Unknown exit-code %s' % code))


#################################
def getHostsNeedingAttention(db):
	lastBackupSQL = ("SELECT MAX(start_time) FROM backups "
			 "WHERE host_id = hosts.id AND successful = 't'")
	backupCountSQL = ("SELECT COUNT(*) FROM backups "
			 "WHERE host_id = hosts.id AND successful = 't'")
	failureDaysSQL = ("CASE WHEN failure_warn IS NOT NULL "
			"THEN failure_warn "
...
def getThisBackupServer(db):
	return(db.queryone('SELECT * FROM backupservers WHERE hostname = %s',
		      os.uname()[1]))
...
def mountSnapshot(db, snapshot):
	...
	if backupserver['id'] != backup['backupserver_id']:
		raise ValueError('ERROR: webcmd is running on the wrong server (%d, '
				'expected %d)' % ( backupserver['id'], backup['backupserver_id'] ))

	#  Is it the most current one, but with no snapshot?
	snapshotFSName = '%s/%s@%s' % ( backupserver['backup_filesystem'],
			host['hostname'], backup['snapshot_name'])
	fp = os.popen('zfs list -t snapshot "%s" 2>&1' % snapshotFSName, 'r')
	fp.read()
	ret = fp.close()
	if ret != None:
		row = db.queryone('SELECT MAX(id) FROM backups WHERE host_id = %s',
				backup['host_id'])
		if not row or row[0] != backup['id']:
			raise ValueError('ERROR: No snapshot exists for this backup, '
					'but this backup is not the most recent.')
		return('%s/%s' % ( backupserver['backup_mountpoint'], host['hostname'] ))

	#  Create snapshot file-system and clone snapshot
	if (not os.path.exists('%s/%s/snapshots'
			% ( backupserver['backup_mountpoint'], host['hostname'] ))
			or os.system('zfs list -t filesystem "%s/%s/snapshots" >/dev/null 2>&1'
				% ( backupserver['backup_mountpoint'], host['hostname'] )) != 0):
		os.system('zfs create "%s/%s/snapshots"'
				% ( backupserver['backup_filesystem'], host['hostname'] ))
	if not os.path.exists('%s/%s/snapshots/%s'
			% ( backupserver['backup_mountpoint'], host['hostname'],
				backup['snapshot_name'] )):
		os.system('zfs clone "%s/%s@%s" "%s/%s/snapshots/%s"'
				% ( backupserver['backup_filesystem'], host['hostname'],
					backup['snapshot_name'], backupserver['backup_filesystem'],
					host['hostname'], backup['snapshot_name'] ))
	return('%s/%s/snapshots/%s' % ( backupserver['backup_mountpoint'],
			host['hostname'], backup['snapshot_name'] ))
...
def updateKey(db, hostname):
	...
	if db.queryone('SELECT COUNT(*) FROM hosts WHERE hostname = %s',
			hostname)[0] == 0:
		db.query("INSERT INTO hosts ( hostname, backupserver_id ) "
	...
	bwlimitOption = (host['rsync_bwlimit']
	...
	fp.write('no-pty,no-agent-forwarding,no-X11-forwarding,'
...
def hostDirs(db, hostname):
	...
	hostDir = AttrString(os.path.join(backupserver['backup_mountpoint'],
			hostname))
...
def runZfsDestroy(filesystem, args = tuple(), retries = 0):
	...
	while True:
		...
		if pipe.returncode == 0 or retries <= 0:
			...
			if stderr:
				#  "dataset is busy" happens sometimes and will get cleaned up later
...
def destroyHost(db, hostname, verbose = True):
	...
	backupserver = db.queryone('SELECT * FROM backupservers WHERE hostname = %s',
			os.uname()[1])
	if backupserver['id'] != host['backupserver_id']:
		hostserver = db.queryone("SELECT hostname FROM backupservers "
				"WHERE id = %s", host['backupserver_id'])[0]
		sys.stderr.write('ERROR: Host "%s" is backed up on server %s\n'
				% ( hostname, hostserver ))
		sys.exit(1)
	...
	for backup in db.query("SELECT * FROM backups WHERE host_id = %s",
			host['id']):
	...
	if verbose:
		...
		sys.stdout.flush()
	runZfsDestroy('%s/%s' % ( backupserver['backup_filesystem'],
			hostname, ), retries = 6)
...
####################################################################
def lookupBackupServer(db, backupservername = None, backupid = None,
		clienthostname = None):
	if clienthostname != None:
		return db.queryone("SELECT backupservers.* "
				"FROM backupservers, hosts "
...
def runWebCmd(backupserverrecord, command):
	...
	sshcmd = [ 'ssh', '-i', '/usr/local/lib/tummy-backup/www/keys/identity',
			  'root@%s' % backupserverrecord['hostname'],
			  '/usr/local/lib/tummy-backup/sbin/webcmd' ]
	pipe = subprocess.Popen(sshcmd, stdout = subprocess.PIPE,
...
def destroyBackup(db, backupid, verbose = True):
	...
	if backupserver['id'] != backup['backupserver_id']:
		...
		backupserverrecord = db.queryone("SELECT backupservers.* "
				  "FROM backupservers, backups "
				  "WHERE backups.id = %s "
						"AND backupservers.id = backups.backupserver_id",
				  backupid)
		...
		if verbose:
			print ('Destroying backup id=%(id)s snapshot=%(snapshot_name)s'
					% backup) + (' on remote server=%s'
							% backupserverrecord['hostname'])
			sys.stdout.flush()
	...
	if verbose:
		...
		sys.stdout.flush()
	runZfsDestroy('%s/%s@%s' % ( backupserver['backup_filesystem'],
			host['hostname'], backup['snapshot_name'] ),
			args = ('-R',), retries = 6)
...
def clearDeadBackups(db, hostrecord = None):
	backupserver = getThisBackupServer(db)
	if hostrecord:
		backups = db.query("SELECT * FROM backups "
				"WHERE host_id = %s AND backupserver_id = %s "
				"AND backup_pid IS NOT NULL", hostrecord['id'], backupserver['id'])
	else:
		backups = db.query("SELECT * FROM backups "
	...
	for backup in backups:
		try:
			...
			db.query("UPDATE backups SET backup_pid = NULL, successful = 'f', "
					"rsync_returncode = -4 WHERE id = %s", backup['id'])
			db.query("UPDATE backups SET end_time = NOW() "
...
def update_usage_stats(db, backupserver, host):
	...
	fp = os.popen("zfs get usedbydataset,usedbysnapshots,compressratio '%s'" %
			os.path.join(backupserver['backup_filesystem'], host['hostname']), 'r')
	data = {}
	...
	if ('usedbydataset' not in data or 'usedbysnapshots' not in data
		...
		sys.stderr.write('Missing item in data=%s for host %s\n'
				% ( repr(data), host['hostname'] ))
		return

	row = db.queryone("SELECT end_time, start_time FROM backups "
	...
	usage_id = db.queryone('SELECT id FROM backupusage WHERE host_id = %s '
			'AND sample_date = NOW()::DATE', host['id'])
	try:
		if usage_id != None:
			db.query("UPDATE backupusage "
			...
			db.query("INSERT INTO backupusage "
