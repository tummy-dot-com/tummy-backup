...
def contextFromLocals(locals):
    ...
    data = { 'url' : url, 'cherrypy' : cherrypy,
            'current_dashboard' : {},
            'current_config' : {},
...
def validateExcludeRule(s):
    import re
    if not re.match(r'^(include|exclude|merge|dir-merge|merge|\+|1)\s+\S.*$',
            s):
...
def validateFailureWarn(s):
    ...
    if s == '': return None
    try:
        row = db.queryone("SELECT %s::INTERVAL", s)
    except DataError:
        raise ValueError('Must be empty (to use default) '
...
def validateServername(s):
    db = dbconnect()
    row = db.queryone("SELECT COUNT(*) FROM backupservers WHERE hostname = %s",
            s)
    if row[0] != 1:
...
class NewHostValidator(Schema):
    hostname = validators.Wrapper(validate_python = validateHostname,
            not_empty = True)
    servername = validators.Wrapper(validate_python = validateServername,
            not_empty = True)
...
class HostConfigValidator(Schema):
    ...
    new_exclude_priority = ForEach(validators.Int(min = 0, max = 9,
            not_empty = True), convert_to_list = True)
    new_exclude_rule = ForEach(
            validators.Wrapper(validate_python = validateExcludeRule,
            not_empty = True), convert_to_list = True)
    ...
    failure_warn = validators.Wrapper(validate_python = validateFailureWarn,
            not_empty = False)
...
class Root(object):
    ...
    def index(self):
        ...
        hosts = list(db.query("SELECT *, "
                ...
                "FROM hosts "
                "ORDER BY hostname"))
        active_backups = list(db.query("SELECT * FROM backups, hosts "
        ...
        for row in db.query("SELECT "
              "sample_date, AVG(usage_pct) AS usage_pct, "
    ...
    def sysconfig(self, **kwargs):
        ...
        if cherrypy.request.method == 'POST' and not errors:
            for field in ['mail_to', 'failure_warn']:
                db.query("UPDATE config SET %s = %%s" % field,
                        formdata[field])
            db.commit()
    ...
    def hostsearch(self, **kwargs):
        ...
        hosts = list(db.query("SELECT hostname FROM hosts "
                "WHERE hostname ~* %s", hostname))

        #  redirect if only one match
        if len(hosts) == 1:
            raise cherrypy.HTTPRedirect(url(str('/hosts/%s/'
                    % hosts[0]['hostname'])))
    ...
    def host(self, hostname):
        ...
        server = db.queryone("SELECT * from backupservers WHERE id = %s",
                host['backupserver_id'])
        backups = db.query("SELECT backups.* FROM backups, hosts "
        ...
        yticks = '['
        sizel = [ [0, 'B'], [1024, 'KiB'], [1024**2, 'MiB'], [1024**3, 'GiB'],
                [1024**4, 'TiB'], [1024**5, 'PiB'], [1024**6, 'EiB'], ]
        for size, ext in sizel:
            if maxSize >= size:
                order = size
                suffix = ext
        if maxSize > 0: val = float(maxSize) / float(order)
        else: val = 0
        tweak = 10
        rounding = 3
        deci = 1
        ...
        yticks += "[0, '0'],"
        yticks += "[%d,'%0.*f %s']," % ( peak * order * 0.25, deci,
                peak * 0.25, suffix )
        yticks += "[%d,'%0.*f %s']," % ( peak * order * 0.5, deci,
                peak * 0.5, suffix )
        yticks += "[%d,'%0.*f %s']," % ( peak * order * 0.75, deci,
                peak * 0.75, suffix )
    ...
    def hostconfig(self, hostname, **kwargs):
        ...
        excludes = db.query("SELECT * FROM excludes "
        ...
        while (kwargs.get('new_exclude_priority')
            ...
            kwargs['new_exclude_priority'] = kwargs['new_exclude_priority'][:-1]
        ...
        errors, formdata = processform(HostConfigValidator(),
                dict(new_exclude_priority = '', new_exclude_rule = '',
                    **dict(host)),
                dict([ (key, kwargs[key]) for key in kwargs.keys()
                    if not key.startswith('delete_') ]),
                [ 'new_exclude_priority', 'new_exclude_rule' ])

        if cherrypy.request.method == 'POST' and not errors:
            for field in ['active', 'use_global_excludes', 'retain_daily',
                    'retain_weekly', 'retain_monthly', 'rsync_do_compress',
                    'window_start_time', 'window_end_time', 'failure_warn']:
                db.query("UPDATE hosts SET %s = %%s WHERE id = %%s" % field,
                        formdata[field], host['id'])
            if (formdata['new_exclude_priority']
                    and formdata['new_exclude_rule']):

                for priority, rule in zip(formdata['new_exclude_priority'],
                        formdata['new_exclude_rule']):
                    db.query("INSERT INTO excludes "
                            "( host_id, priority, rsync_rule ) "
                            "VALUES ( %s, %s, %s )",
                            host['id'], priority, rule)

            deleteList = [int(x.split('_', 1)[1]) for x in kwargs.keys()
                    if x.startswith('delete_')]
            for deleteId in deleteList:
                db.query("DELETE FROM excludes "
            ...
            if output.template_error():
                cherrypy.session['flash'] = ('Error saving settings: %s'
        ...
        import syslog; syslog.syslog('kwargs: %s' % repr(kwargs))
    ...
    def hostdestroy(self, hostname, **kwargs):
        ...
        if cherrypy.request.method == 'POST':
            ...
            if output.template_error():
                cherrypy.session['flash'] = ('Error destroying host: %s'
                        % output.template_error())
    ...
    def backupdestroy(self, hostname, backupid, **kwargs):
        ...
        if cherrypy.request.method == 'POST':
            ...
            if output.template_error():
                cherrypy.session['flash'] = ('Error destroying backup: %s'
                        % output.template_error())
    ...
    def backupctl(self, hostname, action):
        ...
        if output.template_error():
            cherrypy.session['flash'] = ('Error sending command: %s'
                    % output.template_error())
        else:
            cherrypy.session['flash'] = ('Backup "%s" message sent'
                    % command[:-6])
    ...
    def newbackup(self, **kwargs):
        ...
        errors, formdata = processform(NewHostValidator(),
                dict(hostname = '', servername = ''), kwargs)
        ...
        if cherrypy.request.method == 'POST' and not errors:
            ...
            if output.template_error():
                cherrypy.session['flash'] = ('Error creating backup: %s'
                        % output.template_error())

            raise cherrypy.HTTPRedirect(url(str('/hosts/%s/config'
                    % formdata['hostname'])))
        ...
        try:
            min_usage = min([ x['usage_pct'] for x in backupservers
                    if x['usage_pct'] != None ])
            [ x for x in backupservers if x['usage_pct'] == min_usage
                    ][0]['selected'] = True
        except ValueError:
            min_usage = None
...
def checkpassword(realm, user, passwd):
    ...
    db = dbconnect()
    data = db.query('SELECT cryptedpassword FROM users '
            'WHERE name = %s AND cryptedpassword IS NOT NULL', user)
    if data: data = list(data)
    if not data or not data[0]: return(False)
...
def routes():
    ...
    d.connect('backup', r'/hosts/{hostname}/{backupid}/', root,
            action = 'backup')
    d.connect('backuplogfiles', r'/hosts/{hostname}/{backupid}/logfiles', root,
            action = 'backuplogfiles')
    d.connect('backupajax', r'/hosts/{hostname}/{backup}/ajax', root,
            action = 'backupajax')
    d.connect('backuprecover', r'/hosts/{hostname}/{backupid}/recover', root,
            action = 'backuprecover')
    d.connect('backupdestroy', r'/hosts/{hostname}/{backupid}/destroy', root,
    ...
    d.connect('hostconfig', r'/hosts/{hostname}/config', root,
            action = 'hostconfig')
    d.connect('hostdestroy', r'/hosts/{hostname}/destroy', root,
            action = 'hostdestroy')
    d.connect('hostkeys', r'/hosts/{hostname}/keys', root,
            action = 'hostkeys')
    d.connect('backupctrl', r'/hosts/{hostname}/backupctl', root,
            action = 'backupctl')
    d.connect('newbackup', r'/newbackup', root, action = 'newbackup')
    d.connect('detailedindex', r'/index-detailed/', root,
            action = 'detailedindex')
...
if __name__ == '__main__':
    ...
    application = cherrypy.Application(None, script_name='/tummy-backup',
            config = cfg)
